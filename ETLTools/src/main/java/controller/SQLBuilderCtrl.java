...
public class SQLBuilderCtrl {
        ...
        viewAddressSource.getRemoveIPSource().addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedValues != null && !selectedValues.isEmpty()) {
                    if (JOptionPane.showConfirmDialog(parentRootFrame, "Confirma a remoção dos itens selecionados?", "Confirmação", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
    ...
    private void connectSourceDatabase() throws SQLException, ClassNotFoundException {
        ...
        if (view.getIphostSource().getSelectedIndex() < 0) {
            ...
            ParamDatabase param = new ParamDatabase((SGDB) view.getSgbdSource().getSelectedItem(), view.getIphostSource().getSelectedItem().toString(), view.getPortSource().getText(), view.getUserSource().getText(), new String(view.getPasswordSource().getPassword()), view.getDatabaseSource().getText());
